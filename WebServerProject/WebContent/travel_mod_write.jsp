...
<body>
 ...
 String name = ""; String api = ""; String info = ""; String hash1 = "";String hash2 = "";String hash3 = "";String hash4 = "";
 ...
 try{
     ...
     ResultSet rs1 = stmt.executeQuery("select * from travel where code ="+code+"");
     if(rs1.next()){
    	 name = rs1.getString(2); api = rs1.getString("api");
    	 info = rs1.getString("Info"); hash1 = rs1.getString("hash1");hash2 = rs1.getString("hash2");
    	 hash3 = rs1.getString("hash3");hash4 = rs1.getString("hash4");
     }
     ResultSet rs3 = stmt.executeQuery("select * from imageaddr where imagecode="+code+"");
     if(rs3.next()){
    	 image1 = rs3.getString("image1");image2 = rs3.getString("image2");image3 = rs3.getString("image3");
    	 image4 = rs3.getString("image4");image5 = rs3.getString("image5");
     }
          ...
          <nav class="navbar navbar-default navbar-fixed-top" role="navigation" id="navbar-scroll">
			...
			<div class ="Login">
				...
				<button type = "button" class ="btn btn-default btn sm dropdown-toggle"" data-toggle="dropdown"><img src ="./imgs/loginicon.png" class="l_login"><span class="caret"></span></button>
				       ...
				       ResultSet rs2 = stmt.executeQuery("select login from users where id = '"+session.getAttribute("_id")+"' ");
...
<!-- write modal -->
  <form action = "uploadtravel_mod.jsp?code=<%=code %>" method = "post" enctype = "multipart/form-data">
  <div class ="top_line">
  <h1>게시글 수정</h1>
  <div class="modal-dialog modal-fullsize">
    <div class="modal-content">
      <div class="modal_write_header">
        <textarea rows = "1" name = "_title" class="form-control" placeholder="제목"><%=name %></textarea>
      </div>
      <div class="modal-body">
      <textarea  rows="10" name = "_info"  class="form-control" placeholder="간단한 코멘트를 적어주세요(200자 이내)"><%=info %></textarea>
      </div>
    <details>
      <summary class ="select_address">지도 설정</summary>
      &nbsp;&nbsp;<span class ="glyphicon glyphicon-road"></span><input type="text" name="_api" value="<%=api %>" size= "80"placeholder="장소명을 입력해주세요 (예:인천대공원, 인하대 서브웨이 등)">
    </details>
	<details>
	  <summary class="select_hash">해쉬태그 선택</summary>
      <div class="modal-hash-select">
  	<div class="form-row align-items-center">
  	<p>내가 고른 해쉬태그: #<%=hash1 %> #<%=hash2 %> #<%=hash3 %> #<%=hash4 %></p>
    <div class="col-auto my-1">
      <label class="mr-sm-2" for="inlineFormCustomSelect">분위기</label>
      <select name = "_hash1" value ="<%=hash1 %>" class="custom-select mr-sm-2" id="inlineFormCustomSelect">
        <option value="선택안함" selected>선택안함</option>
        <option value="술생각">술생각</option>
        <option value="따뜻한">따뜻한</option>
        <option value="이국적인">이국적인</option>
        <option value="자연적인">자연적인</option>
        <option value="다이어트">다이어트</option>      
      </select>
      <label class="mr-sm-2" for="inlineFormCustomSelect">맛</label>
      <select name = "_hash2" class="custom-select mr-sm-2" id="inlineFormCustomSelect">
        <option value="선택안함" selected>선택안함</option>
        <option value="매콤한">매콤한</option>        
        <option value="달콤한">달콤한</option>        
        <option value="시큼한">시큼한</option>        
        <option value="짭짤한">짭짤한</option>        
        <option value="고소한">고소한</option>        
      </select>
      ...
      <select name = "_hash3" class="custom-select mr-sm-2" id="inlineFormCustomSelect">
        ...
        <option value="언제나좋은">언제나 좋은</option>       
      </select>
	...
	<details>
	  <summary class="upload_text">이미지 업로드(수정 시 모두 재업로드 해야합니다.)</summary>
      <div class="modal-image-upload">
      <table cellpadding = "0" cellspacing="0" width="548">
      <tr><td width="1000" algin="left">
      <input type="file" class ="form-control" name="_image1">
      </td></tr>
      <tr><td width="1000" algin="left">
      <input type="file" class ="form-control" name="_image2">
      </td></tr>
      <tr><td width="1000" algin="left">
      <input type="file" class ="form-control" name="_image3">
      </td></tr>
      <tr><td width="1000" algin="left">
      <input type="file" class ="form-control" name="_image4">
      </td></tr>
      <tr><td width="1000" algin="left">
      <input type="file" class ="form-control" name="_image5">
      </td></tr>                  
      </table>
      </div>
      </details>
      ...
      <div class="modal-footer">
        ...
        <a href="travelboard.jsp?code=<%=code%>"><button type="button" class="btn btn-default">닫기</button></a>
