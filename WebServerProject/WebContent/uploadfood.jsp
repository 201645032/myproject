...
ResultSet rs = stmt.executeQuery("select max(code) from food");
try{
 if(rs.next()){
	int code = rs.getInt(1);
	
	code++;
	stmt.executeUpdate("insert into food values("+code+",'"+title+"','"+session.getAttribute("_id")+"','"+api+"','"+info+"','"+hash1+"','"+hash2+"','"+hash3+"','"+hash4+"',0,"+code+")");
	stmt.executeUpdate("insert into foodimageaddr(imagecode) values("+code+")");
