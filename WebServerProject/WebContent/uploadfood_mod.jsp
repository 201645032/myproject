...
String savePath = "C:/Users/USER/Desktop/정찬영/개인프로젝트/WebServerProject/WebContent/imgs/Food";
...
ResultSet rs = stmt.executeQuery("select code from food where name ='"+title+"' "); 
	stmt.executeUpdate("update food set name = '"+title+"', api = '"+api+"', info = '"+info+"', hash1 = '"+hash1+"', hash2 = '"+hash2+"', hash3 = '"+hash3+"',hash4 = '"+hash4+"' where code = "+code+" ");
	for(int j = 1; j<=c_imagecheck; j++){
		stmt.executeUpdate("update foodimageAddr set image"+j+"='"+image[j]+"' where imagecode = "+code+"");	
	}

 response.sendRedirect("foodboard.jsp?code="+a_code);
