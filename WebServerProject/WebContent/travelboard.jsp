    ...
    <div id="menu_wrap" class="bg_white" style="width:20%;height:70%;">
      ...
      if(rs8.next()){
    	  ...
    	  if(w_id.equals(session.getAttribute("_id"))){
    		  %>
    		  <a href="travel_mod_write.jsp?code=<%=code %>"><button type="button" class="btn btn-primary">수정</button></a>
    		  <a href="travel_remove_write.jsp?code=<%=code %>"><button type="button" class="btn btn-primary">삭제</button></a>
    		  
    	  <% }else%>
    	  <a href="travel.jsp"><button type="button" class="btn btn-primary">닫기</button></a>
      <% }%>
      </div>
    </div> <!-- 모달 콘텐츠 -->
  </div> <!-- 모달 다이얼로그 -->
